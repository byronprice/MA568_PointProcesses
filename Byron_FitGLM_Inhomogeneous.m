function [Results] = Byron_FitGLM_Inhomogeneous(Process,X,testInds,modelName)
%Byron_FitGLM
...
numCoeffs = size(X,2)+1;
numProcesses = length(Process);
Results = zeros(numProcesses,numCoeffs,4);
for ii=1:numProcesses
    [b,dev,stats] = glmfit(X,Process{ii}','poisson');
    for jj=1:numCoeffs
...
%         e.Color = myBlue;hold on;e.LineStyle = 'none';
   ...
   elseif ii <= 6
       subplot(1,3,ii-3);e = errorbar(0:(numCoeffs-1),squeeze(exp(Results(testInds(ii),:,1))),...
            2.*squeeze(expError(testInds(ii),:)),'s','LineWidth',2);axis([-1 numCoeffs minB-1 maxB+1]);
        e.Color = myGreen;hold on;e.LineStyle = 'none';
       % plot(0:(numCoeffs-1),signifResults(testInds(ii),:),'*','Color',myGreen);
   elseif ii <= 9
       subplot(1,3,ii-6);e = errorbar(0:(numCoeffs-1),squeeze(exp(Results(testInds(ii),:,1))),...
            2.*squeeze(expError(testInds(ii),:)),'d','LineWidth',2);axis([-1 numCoeffs minB-1 maxB+1]);
...
for ii=1:length(testInds)
   if ii <= 6 && ii > 3
       for jj=0:(numCoeffs-1)
           if jj==0
               subplot(1,3,ii-3);plot(jj,signifResults(testInds(ii),jj+1),'*','Color',myGreen);
           elseif Results(testInds(ii),jj+1,1) > 0
                subplot(1,3,ii-3);plot(jj,signifResults(testInds(ii),jj+1),'^','Color',myGreen);
           elseif Results(testInds(ii),jj+1,1) < 0 
                subplot(1,3,ii-3);plot(jj,signifResults(testInds(ii),jj+1),'v','Color',myGreen);
           end
       end
       set(gca,'FontSize',12);
   elseif ii <= 9 && ii > 6
       for jj=0:(numCoeffs-1)
           if jj==0
               subplot(1,3,ii-6);plot(jj,signifResults(testInds(ii),jj+1)-0.1,'*','Color',myRed);
           elseif Results(testInds(ii),jj+1,1) > 0
                subplot(1,3,ii-6);plot(jj,signifResults(testInds(ii),jj+1)-0.1,'^','Color',myRed);
           elseif Results(testInds(ii),jj+1,1) < 0
                subplot(1,3,ii-6);plot(jj,signifResults(testInds(ii),jj+1)-0.1,'v','Color',myRed);
           end
       end
       set(gca,'FontSize',12);
   end
